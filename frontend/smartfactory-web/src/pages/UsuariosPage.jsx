...
function getRolesDisponiveis(role) {
    ...
    // gerente pode criar gerente ou operador
    return [
        { value: "gerente", label: "Gerente" },
        { value: "operador", label: "Operador" },
    ];
...
export default function UsuariosPage({ auth }) {
    ...
    const handleCriar = async (values) => {
        const roleEscolhido = values.role || "operador";
        const ehAdmin = roleEscolhido === "admin";

        if (!ehAdmin && !empresa) return fb("erro", "Selecione uma empresa primeiro.");
        ...
        try {
            await usuariosAPI.criar({
                ...
                senha: values.senha,
                role: roleEscolhido,
                empresa_id: ehAdmin ? null : empresa.id,
            });
            fb("ok", ehAdmin
                ? `Admin "${values.nome}" criado!`
                : `Usuário "${values.nome}" criado em ${empresa.nome}!`);
            setModalCriar(false);
            refetch();
