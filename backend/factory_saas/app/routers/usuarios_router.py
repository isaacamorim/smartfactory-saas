...
def criar_usuario(
    ...
    """
    Admin:   pode criar admin (sem empresa), gerente ou operador.
    Gerente: pode criar gerente ou operador, apenas para a própria empresa.
             Não pode criar admins.
    """
    # Ninguém cria admin a não ser outro admin
    if data.role == RoleEnum.admin and current_user.role != RoleEnum.admin:
        raise HTTPException(status_code=403, detail="Apenas admins podem criar outros admins")

    # Admin sem empresa ao criar gerente — empresa obrigatória
    if data.role == RoleEnum.admin:
        data.empresa_id = None   # admin nunca tem empresa fixa
    ...
    # Gerente e operador precisam de empresa
    if data.role in (RoleEnum.gerente, RoleEnum.operador) and not data.empresa_id:
        raise HTTPException(status_code=400, detail=f"{data.role.value.capitalize()} precisa estar vinculado a uma empresa")
...
def atualizar_usuario(
    ...
    # Gerente só edita usuários da própria empresa e não pode promover para admin
    if current_user.role == RoleEnum.gerente:
        if usuario.empresa_id != current_user.empresa_id:
            raise HTTPException(status_code=403, detail="Acesso negado")
        if data.role == RoleEnum.admin:
            raise HTTPException(status_code=403, detail="Gerente não pode promover para admin")
...
def deletar_usuario(
    ...
    if current_user.role == RoleEnum.gerente:
        ...
        if usuario.role == RoleEnum.admin:
            raise HTTPException(status_code=403, detail="Gerente não pode desativar admins")
